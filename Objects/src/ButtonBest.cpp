#include "../include/ButtonBest.h"

ButtonBest::ButtonBest(const std::string& parse_str) {
...
void ButtonBest::CorrectSize() const {
    if (image_defined) {
...
void ButtonBest::OnFrame() {
    CorrectSize();
}

void ButtonBest::OnDraw(sf::RenderWindow& window) {
    if (image_defined) {
        window.draw(*sprite);
    } else {
...
void ButtonBest::OnEvent(sf::Event& event, sf::RenderWindow& window) {

...
NewButton::NewButton(const std::string& data_string) {
    ...
    if (!data.image_path.empty()) {
        ...
        rect_width = width - data.outline_thickness_in_pixels * 2;
        rect_height = height - data.outline_thickness_in_pixels * 2;
...
void NewButton::CorrectSize() const {
    ...
    if (text_defined) {
        ...
        float padding_y = data.character_size_in_pixels * scale - text->getGlobalBounds().height;
